...
int main(void)
{
  ...
  while (1)
  {
		  ...
		  checkfinal = check(hum, temp);
		  temp_decimals = abs((int)temp % 10);
		  hum_decimals = abs((int)hum % 10);
		  temp_degree = temp / 10;
		  hum_int = hum / 10;

		  sprintf(buf5,"checks: %d, checkfinal: %d \n\r",checks, checkfinal);	//test
		  for(int i=0;i<50;i++){
			  USART2_write(buf5[i]);
		  }
		   ...
		   */
		  sprintf(buf,"temperature: %d.%d \n\r",temp_degree, temp_decimals);
		  for(int i=0;i<20;i++){
			  USART2_write(buf[i]);
		  }


		  sprintf(buf2,"humidity: %d.%d \n\r",hum_int, hum_decimals);
		  for(int i=0;i<20;i++){
			  USART2_write(buf2[i]);
		  }

		  sprintf(buf3,"checksum: %d \n\r",checks);
		  for(int i=0;i<20;i++){
			  USART2_write(buf3[i]);
		  }

		  if(checkfinal == checks){
			  sprintf(buf4,"checksum is correct \n\r");
			  for(int i=0;i<50;i++){
				  USART2_write(buf4[i]);
			  }
		  }
...
void read_dht22_humidity_and_temperature(int *hum, int *temp, int *checks)
{
	...
	unsigned long long mask=0x8000000000;		// mask will be shifted from right to left for 40 times
	...
	while(i<40)
	{
		...
		delay_Us(35);
		// the bits sent from the right to the left that means the humidity is the last 16 bits and checksum is the first
		// 8 bits transfered from dht22 to output register of stm32
		if((GPIOA->IDR & 0x40)&& i<16)
		{
			humidity=humidity|(mask>>24);
		}
		if((GPIOA->IDR & 0x40)&& i>=16 && i<32)
		{
			temperature=temperature|(mask>>8);
		}
